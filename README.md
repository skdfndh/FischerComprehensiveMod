...
- 自动投放窝料：连续投放现有窝料并叠加持续时间；优先使用购买的区域窝料，再使用原版定时恢复、上限为 5 的免费窝料。
- 神奇鱼缸整理：将收益更高的鱼放入神奇鱼缸；被替换下来的鱼和伙伴任务所需的鱼都会保留在鱼篓。
- 自动完成鱼群聚集：检测到小游戏出现后自动完成。
- 自动伙伴交互：自动处理普通对话、接取伙伴任务，并在材料齐全时提交。
- 鱼篓满时自动出售：按全选规则出售一次，并保留锁定鱼、任务鱼和受保留设置保护的鱼。
- 加速后的商店刷新与自动采购同步。
...
| `Auto sprinkle bait` | 连续投放现有窝料并叠加时间。不会自动购买窝料。 |
...
| `Auto sell full basket` | 鱼篓达到容量时自动按全选规则出售一次。 |
